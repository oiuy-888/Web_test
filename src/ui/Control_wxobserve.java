...
public class Control_wxobserve extends Control{

	public Control_wxobserve(){
		final JFrame jframe = new JFrame("万象动察");
		jframe.setBounds(305, 0, 700, 600);
		jframe.setVisible(true);
		jframe.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
		JPanel pane = new JPanel();
		JLabel[] casename = new JLabel[src4.length];
		final JTextField[] casetime = new JTextField[src4.length];
		GridLayout gridLayout = new GridLayout(100, 2, 300, 0);
		pane.setLayout(gridLayout);
		for(int i=0;i<src4.length;i++){
			//case名称
			casename[i] = new JLabel("case"+casedesc[i+24]);
			pane.add(casename[i]);
			//case次数
			casetime[i] = new JTextField("1");
			casetime[i].setEditable(true);
			casetime[i].setText(String.valueOf(wxobserve[i]));
			pane.add(casetime[i]);
		}
		...
		JButton button=new JButton(new AbstractAction("确定") {	
			...
			public void actionPerformed(ActionEvent e) {
				for(int i=0;i<src4.length;i++){
					wxobserve[i]=Integer.parseInt(casetime[i].getText());
				}
				jframe.dispose();
			}
		});
