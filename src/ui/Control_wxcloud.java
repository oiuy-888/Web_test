...
public class Control_wxcloud extends Control{
	
	public Control_wxcloud(){
		final JFrame jframe = new JFrame("万象云盾");
		jframe.setBounds(305, 0, 700, 600);
		jframe.setVisible(true);
		jframe.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
		JPanel pane = new JPanel();
		JLabel[] casename = new JLabel[src5.length];
		final JTextField[] casetime = new JTextField[src5.length];
		GridLayout gridLayout = new GridLayout(100, 2, 300, 0);
		pane.setLayout(gridLayout);
		for(int i=0;i<src5.length;i++){
			//case名称
			casename[i] = new JLabel("case"+casedesc[i+28]);
			pane.add(casename[i]);
			//case次数
			casetime[i] = new JTextField("1");
			casetime[i].setEditable(true);
			casetime[i].setText(String.valueOf(wxcloud[i]));
			pane.add(casetime[i]);
		}
		...
		JButton button=new JButton(new AbstractAction("确定") {	
			...
			public void actionPerformed(ActionEvent e) {
				for(int i=0;i<src5.length;i++){
					wxcloud[i]=Integer.parseInt(casetime[i].getText());
				}
				jframe.dispose();
			}
		});
